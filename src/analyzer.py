...
===================================
职责：
封装 LLM 调用逻辑（通过 LiteLLM 统一调用 Gemini/Anthropic/OpenAI 等）
结合技术面和消息面生成分析报告
解析 LLM 响应为结构化 AnalysisResult
"""
import json
import logging
import math
...
def init(self, message: str, *, partial_received: bool = False):
super().init(message)
...
trend_result.dict if hasattr(trend_result, "dict") else {}
)
...
def capital_flow_bias_with_status(
...
normalized_status = status.replace("-", " ").replace("", " ").strip()
...
多来源获取股票中文名称
获取策略（按优先级）：
从传入的 context 中获取（realtime 数据）
从静态映射表 STOCK_NAME_MAP 获取
从 DataFetcherManager 获取（各数据源）
返回默认名称（股票 + 代码）
Args:
stock_code: 股票代码
context: 分析上下文（可选）
...
name: str
========== 核心指标 ==========
sentiment_score: int # 综合评分 0-100 (>70 强烈看多，>60 看多，40-60 震荡，<40 看空)
...
short_term_outlook: str = "" # 短期展望（1-3 日）
medium_term_outlook: str = "" # 中期展望（1-2 周）
========== 技术面分析 ==========
...
Gemini AI 分析器
职责：
调用 Google Gemini API 进行股票分析
结合预先搜索的新闻和技术面数据生成分析报告
解析 AI 返回的 JSON 格式结果
使用方式：
...
e.get('model_name', '').startswith('_legacy') for e in config.llm_model_list
)
@staticmethod
def _legacy_router_provider_alias(model: str) -> str:
provider = model.split("/", 1)[0] if "/" in model else "openai"
return f"_legacy{provider}__"
@staticmethod
...
last_error = RuntimeError(f"{type(exc).name}: {safe_error}")
except Exception as exc:
safe_error = self._sanitize_litellm_exception_text(exc, config=config, model=model)
logger.warning(
...
last_error = RuntimeError(f"{type(e).name}: {safe_error}")
continue
...
logger.info(f"[LLM 调用] 开始调用 {model_name}...")
...
记录响应信息
logger.info(
f"[LLM 返回] {model_name} 响应成功，耗时 {elapsed:.2f} s, 响应长度 {len (response_text)} 字符"
)
if backend_id in LOCAL_CLI_GENERATION_BACKEND_IDS:
response_preview = redact_diagnostic_text (response_text, limit=300)
else:
response_preview = response_text [:300] + "..." if len (response_text) > 300 else response_text
logger.info(f"[LLM 返回 预览]\n {response_preview}")
if backend_id not in LOCAL_CLI_GENERATION_BACKEND_IDS:
logger.debug (
...
logger.warning (
"[LLM 完整性] 必填字段缺失 % s，已占位补全，不阻塞流程",
missing_fields,
)
break
if should_persist_usage_telemetry(llm_usage):
persist_llm_usage(llm_usage, model_used, call_type="analysis", stock_code=code)
logger.info(f"[LLM 解析] {name}({code}) 分析完成: {result.trend_prediction}, 评分 {result.sentiment_score}")
return result
except Exception as e:
...
60 日涨跌幅	{rt.get('change_60d', 'N/A')}%	中期表现
"""
添加财报与分红（价值投资口径）
...
近 12 个月每股现金分红	{ttm_cash}	仅现金分红、税前口径
TTM 股息率	{ttm_yield}	公式：近 12 个月每股现金分红 / 当前价格 × 100%
TTM 分红事件数	{ttm_count}	
...
5 日净流入	{stock_flow.get('inflow_5d', 'N/A')}	用于判断资金持续性
10 日净流入	{stock_flow.get('inflow_10d', 'N/A')}	用于判断资金持续性
...
平均成本	{chip.get ('avg_cost', 'N/A')} 元	现价应高于 5-15%
...
if use_legacy_default_prompt:
bias_warning = "🚨 超过 5%，严禁追高！" if trend.get ('bias_ma5', 0) > 5 else "✅ 安全范围"
prompt += f"""
趋势分析预判（基于交易理念）
指标	数值	判定
趋势状态	{trend.get('trend_status', unknown_text)}	
均线排列	{trend.get('ma_alignment', unknown_text)}	MA5>MA10>MA20 为多头
...
if math.isfinite (parsed_volume_change) and parsed_volume_change > 10:
prompt += """
⚠️ 量能异常提示：成交量较昨日放大超过 10 倍，可能受异常数据或一次性冲量影响，必须降权解读，不能机械视为强确认信号
"""
添加新闻搜索结果（重点区域）
...
业绩预期基于指数成分股整体表现，而非基金公司财报
risk_alerts 中不得出现基金管理人相关的公司经营风险
"""
prompt += f"""
⚠️ 重要：输出正确的股票名称格式
正确的股票名称格式为 “股票名称（股票代码）”，例如 “贵州茅台（600519）”。
如果上方显示的股票名称为 "股票 {code}" 或不正确，请在分析开头明确输出该股票的正确中文全称。
"""
if use_legacy_default_prompt:
prompt += f"""
重点关注（必须明确回答）：
❓ 是否满足 MA5>MA10>MA20 多头排列？
❓ 当前乖离率是否在安全范围内（<5%）？—— 超过 5% 必须标注 "严禁追高"
❓ 量能是否配合（缩量回调 / 放量突破）？
❓ 筹码结构是否健康？
❓ 消息面有无重大利空？（减持、处罚、业绩变脸等）
"""
else:
prompt += f"""
重点关注（必须明确回答）：
❓ 当前结构是否满足激活技能的关键触发条件？
❓ 当前入场位置与风险回报是否合理？若偏离过大，请明确说明等待条件
❓ 量能、波动与筹码结构是否支持当前结论？
❓ 消息面有无重大利空或与技能结论冲突的信息？
❓ 若结论成立，具体触发条件、止损位、观察点分别是什么？
"""
prompt += f"""
决策仪表盘要求：
股票名称：必须输出正确的中文全称（如 "贵州茅台" 而非 "股票 600519"）
核心结论：一句话说清该买 / 该卖 / 该等
